...
#[test]
fn update_with_two_args_is_error() {
    // Section is now required, so 2 args no longer means id+entry.
    assert!(Cli::try_parse_from(["devlogger", "update", "1", "fixed"]).is_err());
}
